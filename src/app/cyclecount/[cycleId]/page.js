    ...
    try {
      ...
      for (const unit of processedUnits) {
        ...
        if (unit.quantity !== null && unit.quantity !== undefined) {
          payload.count_quantity = unit.quantity;
        }

        if (unit.batch_no) {
          payload.count_batch_no = unit.batch_no;
        }

        if (unit.expiry_date) {
          payload.count_expiry_date = dayjs(unit.expiry_date).format("YYYY-MM-DD");
        }

        if (unit.mrp !== null && unit.mrp !== undefined) {
          payload.mrp = unit.mrp;
        }
      ...
      const zeroSystemBatches = selectedProduct.systemUnits.filter(
        (u) => Number(u.sys_quantity || 0) <= 0 && // ✅ includes 0 + negative
          u.status !== "submitted",
      );
...
const tabCounts = useMemo(() => {
  return {
    ...
    pending: products.filter((p) => {
      // must have pending
      const hasPending = p.systemUnits.some(
        (u) => u.status === "pending"
      );

      if (!hasPending) return false;

      // ✅ apply toggle ONLY here
      if (hideZeroStock) {
        return p.systemUnits.some(
          (u) =>
            u.status === "pending" &&
...
const filteredItems = useMemo(() => {
  ...
  if (activeTab === "all") {
    ...
    data = products.filter((p) => {
      return p.systemUnits.some((u) => {
        // Only pending items
        if (u.status !== "pending") return false;

        // Apply toggle filter
        if (hideZeroStock) {
          return Number(u.sys_quantity || 0) > 0;
        }
...
}, [products, activeTab, searchText, hideZeroStock]); // ✅ IMPORTANT
        ...
        >
          ...
          >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <Text style={{ fontSize: 12 }}>Hide Zero Stock</Text>

              <Switch
                size="small"
                checked={hideZeroStock}
                onChange={(val) => setHideZeroStock(val)}
              />
            </div>
