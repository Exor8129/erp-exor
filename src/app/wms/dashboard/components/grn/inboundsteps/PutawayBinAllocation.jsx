...
// 1. Fetch Rack Levels, Container Items, and GRN Items
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // 1. Fetch WMS Rack Levels master
      ...
      // 2. Fetch GRN items for the active GRN
      ...
      // Collect IDs of GRN items for this specific GRN
      const grnItemIds = (grnItemsData || []).map((item) => item.id);

      // If this GRN has no items, reset container lists and exit early
      if (grnItemIds.length === 0) {
        setContainerItemsMap({});
        setContainersList([]);
        setAllocations([]);
        return;
      }

      // 3. Fetch ONLY container items linked to this GRN's items
      ...
      const scopedContainers = Object.values(uniqueContainersObj);
      setContainersList(scopedContainers);

      const scopedContainerIds = scopedContainers.map((c) => c.id);

      // 4. Fetch existing container locations ONLY for these GRN containers
      if (scopedContainerIds.length > 0) {
        const { data: existingLocs, error: locError } = await supabase
          ...
          `)
          .in("container_id", scopedContainerIds)
          .eq("status", "STORED");
