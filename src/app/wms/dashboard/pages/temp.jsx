...
import { PlusOutlined, ShopOutlined, DeleteOutlined, CheckOutlined, CloseOutlined } from "@ant-design/icons";
...
export default function WarehouseDesigner() {
    ...
    if (editingElementId) {
      setElements((prevElements) =>
        prevElements.map((el) => {
          ...
          if (fieldName === "type") {
            if (value === "rack") { width = 40; height = 100; }
            else if (value === "s-rack") { width = 30; height = 65; }
            else if (value === "fsa") { width = 50; height = 55; }
            else { width = 40; height = 40; }
          }
          ...
          return {
            ...
            metadata: {
              ...el.metadata,
              custom_label_id: fieldName === "id" ? value : el.metadata?.custom_label_id,
              status: fieldName === "status" ? value : el.metadata?.status,
              orientation: fieldName === "orientation" ? value : el.metadata?.orientation,
              color: fieldName === "color" ? value : el.metadata?.color,
              rotation: fieldName === "rotation" ? Number(value) || 0 : el.metadata?.rotation,
              textX: fieldName === "textX" ? Number(value) || 0 : el.metadata?.textX,
            },
          };
        })
      );
  ...
  const addElement = async () => {
    ...
    if (!customLabel || customLabel.startsWith("OBJ-") || customLabel.startsWith("R") || customLabel.startsWith("SR") || customLabel.startsWith("FSA")) {
      let prefix = "R";
      ...
      const existingWarehouseElements = elements.filter(
        (el) => el.warehouse_id === selectedWarehouse.id && el.type === selectedType
      );
    ...
    basePayload.metadata = {
      ...
      status: ["rack", "s-rack", "fsa", "object"].includes(selectedType) ? elementData.status : "vacant",
      orientation: ["rack", "s-rack", "fsa", "object"].includes(selectedType) ? elementData.orientation : "vertical",
  ...
  const finishEditingElement = async () => {
    ...
    const locallyModifiedElement = elements.find(el => el.id === editingElementId);
  ...
  const handleElementSelect = (item) => {
    ...
    const foundElement = elements.find(
      (el) => el.metadata?.custom_label_id === targetLabel || el.id === targetLabel
    );
  ...
  return (
    <div className="w-full h-screen bg-gray-100 flex flex-col overflow-hidden">
      {/* TOP HEADER SECTION */}
      <div className="h-[70px] bg-white border-b px-6 flex items-center justify-between shrink-0">
        <div>
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* LEFT SIDEBAR PANEL */}
        <div className="w-[280px] shrink-0 border-r bg-white p-4 flex flex-col gap-4 overflow-y-auto">
          <div>
            ...
            {editingElementId ? (
              <div className="flex flex-col gap-2 mt-4 p-2 bg-blue-50 border border-blue-200 rounded-md">
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <Button icon={<CloseOutlined />} block onClick={handleCancelEditing}>
                    Cancel
                  </Button>
                </div>
              </div>
            ) : (
              <Button type="primary" block onClick={addElement} className="mt-2">
                Add Layout Element
              </Button>
            )}
          </div>
      ...
      >
        <Form layout="vertical" form={form} onFinish={createWarehouse}>
          <Form.Item label="Warehouse Name" name="name" rules={[{ required: true, message: "Enter warehouse name" }]}>
            <Input />
          </Form.Item>
          ...
          <Form.Item label="Warehouse Length (meters)" name="length"><Input type="number" /></Form.Item>
          <Form.Item label="Warehouse Width (meters)" name="width"><Input type="number" /></Form.Item>
          <Form.Item label="Warehouse Height (meters)" name="height"><Input type="number" /></Form.Item>
          <Form.Item label="Number of Tiers" name="tiers" initialValue={1}><Input type="number" /></Form.Item>
          <Button type="primary" htmlType="submit" block>Create Warehouse</Button>
        </Form>
      </Modal>
    </div>
  );
